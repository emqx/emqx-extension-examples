...
impl pb::hook_provider_server::HookProvider for HookProviderServer {
    ...
    async fn on_client_check_acl(
        &self,
        req: Request<ClientCheckAclRequest>,
    ) -> HookProviderResult<ValuedResponse> {
        println!("{:#?}", req);
        Ok(Response::new(ValuedResponse {
            r#type: 0,
    ...
    async fn on_session_takeovered(
        &self,
        req: Request<SessionTakeoveredRequest>,
    ) -> HookProviderResult<EmptySuccess> {
        println!("{:#?}", req);
        Ok(Response::new(EmptySuccess {}))
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("HookProviderServer listening on: {}", addr);
